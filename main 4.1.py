...
def starting(message):
    ...
    def mu1(message):
        ...
        markup1.add('Забронировать столик '+emoji["time"],'Меню '+emoji["book"],'О ресторане '+emoji["ab"], 'Рассылка '+emoji["mai"],'Отзывы '+emoji["rev"])
    ...
    def finding_orders(message):
        ...
        order = preorders.find({},{"User ID":1,'Phone':1,'Pre-order': 1, "Time":1, "Status":1})
        markup_ap=types.ReplyKeyboardMarkup(one_time_keyboard=True)
        markup_ap.add('Подтвердить '+emoji["yep"], "Назад "+emoji['back'])
        order_li=list(order)
        print(order_li)
        def backm(message):
        ...
        try:
            ...
            s=bot.send_message(chat_id, ord_txt, reply_markup=markup_ap)
    ...
    def choosing(message):
        ...
        if message.text=="Заблокировать "+emoji["lo"]:
            ...
            s3=bot.send_message(chat_id,'Enter "User ID" to block the user', reply_markup=mu_backk1)
            ...
            s3=bot.send_message(chat_id,'Enter "User ID" to unblock the user', reply_markup=mu_backk2)
    ...
    def blacklis(message):
        ...
        s3=bot.send_message(chat_id,'Enter what do u want to do? ', reply_markup=markup_ch)
    ...
    def mailing(message):
        ...
        if message.text=="ПОДТВЕРДИТЬ":
            telebot=mongo["telebot"]
            users=telebot["users"]
            reg_users = users.find({},{"User Name":1,'User ID': 1, "Mailing": 1})
            for usr in reg_users:
                mail_id=usr["User ID"]
                name_usr=usr["User Name"].title()
                for n in admin_list:
                    if mail_id!=n:
                        if usr["Mailing"]==1:
    ...
    def second(message):
            ...
            if message.text=="Назад "+emoji["back"]:
                ...
                for k in reg_users:
                    if k['User ID']==chat_id:
                         phone=k['Phone']
                         break
                li_order={"User ID":chat_id,'Phone':phone,"Pre-order":user_order, "Time":str(date), "Status":0}
                print(li_order)
        ...
        def photos(message):
            ...
            if message.text=="Назад "+emoji["back"]:
                ...
                markup_back2=types.ReplyKeyboardMarkup(one_time_keyboard=True)
                markup_back2.add("Главное меню", "Где нас найти "+emoji["feet"])
        ...
        if message.text=="Забронировать столик "+emoji["time"]:
            ...
            markup_about.add("Назад "+emoji["back"], "Показать фото "+emoji["cam"], "Где нас найти "+emoji["feet"])
...
def callback_inline(call):
    ...
    try:
        ...
        bot.send_message(call_id,'Issues with Telegram')
        print()
